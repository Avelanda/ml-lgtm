"""
Minimal LGTM inference example using the Flash3D model.
...
"""
Usage: python examples/inference_flash3d.py

Overview:
  Input: 1 RGB image + intrinsics (monocular; poses not used by encoder).
  Output: Textured 2D Gaussians representing the 3D scene.

  Flash3D is a monocular model: only the first context image is processed.
  It predicts depth via UniDepth, then back-projects each pixel into camera
  space using intrinsics. Poses are not used by the encoder. The Gaussians
  live in camera space of view 0, which equals the world frame because
  normalize_scene_scale and relative_pose normalize context[0] to identity.
  ...
  Why 2 context views: the encoder only uses the first image, but the second
  view's pose serves as the endpoint for video interpolation.

  Video interpolation: smoothly interpolates from context[0] (the encoded
  identity view) to context[1] and renders the Gaussians at each step.
...
def main():
    output_dir = Path(__file__).parent / "outputs_lgtm_flash3d"
    ...
    experiment = "dl3dv_flash3d_lgtm_288x512_2304x4096"
    checkpoint_path = str(
        _project_root / "weights/dl3dv_flash3d_lgtm_288x512_2304x4096.ckpt"
    )
    context_indices = [0, 9]
    ...
    # Flash3D only uses the first context image for encoding, but we still.
    # Provide 2 context views so the interpolation video has two poses.
