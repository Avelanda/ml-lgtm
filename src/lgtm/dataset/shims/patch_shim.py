"""
Patch extraction shim for high-resolution views.
...
from lgtm.dataset.data_types import BatchedExample, BatchedViews


def apply_patch_shim_to_views(
    views: BatchedViews, patch_size: int
) -> BatchedViews:
    _, _, _, h, w = views["image"].shape

    # Image size must be even so that naive center-cropping.
    # Does not cause misalignment.
    if h % 2 != 0 or w % 2 != 0:
        raise ValueError(f"Image dimensions must be even, got ({h}, {w})")
...
def apply_patch_shim(batch: BatchedExample, patch_size: int) -> BatchedExample:
    """
    Crop images in the batch so that their dimensions are cleanly divisible
    by the specified patch size.
    """
    return {
        **batch,
