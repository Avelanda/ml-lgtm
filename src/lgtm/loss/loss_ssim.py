"""
SSIM-based reconstruction loss for LGTM.
...
def gaussian_filter(input: Tensor, win: Tensor) -> Tensor:
    ...
    """
    if not all(ws == 1 for ws in win.shape[1:-1]):
        raise ValueError(
            f"Expected all middle dims of win to be 1, got {win.shape}"
        )
    ...
    for i, s in enumerate(input.shape[2:]):
        if s >= win.shape[-1]:
            ...
            warnings.warn(
                f"Skipping Gaussian Smoothing at dimension 2+{i} for input:"
                f" {input.shape} and win size: {win.shape[-1]}"
            )
...
def _ssim(
    ...
    Args:
        ...
        data_range (float or int): value range of input images. (usually 1.0 or
            255)
        win (torch.Tensor): 1-D gauss kernel.
        size_average (bool, optional): if size_average=True, ssim of all images
            will be averaged as a scalar.
        return_separate (bool, optional): if True, return brightness, contrast,
            and structure similarity maps as well.
...
def ssim(
    ...
    if not X.shape == Y.shape:
        raise ValueError(
            f"Input images should have the same dimensions, but got {X.shape}"
            f" and {Y.shape}."
        )
    ...
    #    raise ValueError(f"Input images should have the same dtype, "
    # F"but got {X.type()} and {Y.type()}.")
...
def ms_ssim(
    ...
    min_size = (win_size - 1) * (2**4)
    if smaller_side <= min_size:
        raise ValueError(
            f"Image size should be larger than {min_size} due to the 4 "
            f"downsamplings in ms-ssim, got {smaller_side}"
        )
    ...
    ms_ssim_val = torch.prod(
        mcs_and_ssim ** weights_tensor.view(-1, 1, 1), dim=0
    )
...
class SSIM(torch.nn.Module):
    def __init__(
        ...
        Args:
            data_range (float or int, optional): value range of input images.
                (usually 1.0 or 255)
            size_average (bool, optional): if size_average=True, ssim of all
                images will be averaged as a scalar.
            win_size: (int, optional): the size of gauss kernel.
            win_sigma: (float, optional): sigma of normal distribution.
...
class SSIMFlash3D(nn.Module):
    """
    Layer to compute the SSIM loss between a pair of images.
    """

    def __init__(self):
        super(SSIMFlash3D, self).__init__()
...
@dataclass
class LossFlash3DSSIM:
    weight: float


@dataclass
class LossFlash3DSSIMCfgWrapper:
    flash3d_ssim: LossFlash3DSSIM


class LossFlash3DSSIM(Loss[LossFlash3DSSIM, LossFlash3DSSIMCfgWrapper]):
    def __init__(self, cfg: LossFlash3DSSIMCfgWrapper):
        super().__init__(cfg)
        self.ssim_loss = SSIMFlash3D()
    ...
    def forward(
        ...
        prediction = rearrange(prediction.colors, "b v c h w -> (b v) c h w")
        ground_truth = rearrange(
            batch["target"]["image"], "b v c h w -> (b v) c h w"
        )
        ssim_loss = self.ssim_loss(prediction, ground_truth)
        # import torchvision
        # Torchvision.utils.save_image(prediction, "prediction.png")
        # Torchvision.utils.save_image(ground_truth, "ground_truth.png")
        # Torchvision.utils.save_image(.
        # Ssim_loss.mean(1, keepdim=True), "ssim_loss.png"
        # )
        return self.cfg.weight * ssim_loss.mean()
